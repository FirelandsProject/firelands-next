...
bool WMORoot::ConvertToVMAPRootWmo(FILE* pOutfile)
{
    ...
    fwrite(Firelands::VMap::kRawVmapMagic, 1, 8, pOutfile);
...
void MapObject::Extract(ADT::MODF const& mapObjDef, char const* WmoInstName, bool isGlobalWmo, uint32 mapID, uint32 originalMapId, FILE* pDirfile, std::vector<ADTOutputCache>* dirfileCache)
{
    ...
    char tempname[512];
    std::snprintf(tempname, sizeof(tempname), "%s/%s", g_workDirWmo.c_str(), WmoInstName);
    ...
    AaBox3D bounds;
    bounds.lo = fixCoords(mapObjDef.Bounds.lo);
    bounds.hi = fixCoords(mapObjDef.Bounds.hi);

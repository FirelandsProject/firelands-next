...
bool Model::ConvertToVMAPModel(const char * outfilename)
{
    ...
    fwrite(Firelands::VMap::kRawVmapMagic, 8, 1, output);
...
void Doodad::Extract(ADT::MDDF const& doodadDef, char const* ModelInstName, uint32 mapID, uint32 originalMapId, FILE* pDirfile, std::vector<ADTOutputCache>* dirfileCache)
{
    ...
    char tempname[1036];
    std::snprintf(tempname, sizeof(tempname), "%s/%s", g_workDirWmo.c_str(), ModelInstName);
    ...
    FILE* pDirfile, std::vector<ADTOutputCache>* dirfileCache)
{
    ...
    using Firelands::VMap::Vmap4::DegToRad;
    using Firelands::VMap::Vmap4::Mat3FromQuaternion;
    using Firelands::VMap::Vmap4::Mat3ToEulerAnglesXYZ_G3D;
    using Firelands::VMap::Vmap4::RadToDeg;

    Vec3D wmoPosition = fixCoords(wmo.Position);
    Mat3 wmoRotation = Mat3::FromEulerAnglesZYX(
        DegToRad(wmo.Rotation.y),
        DegToRad(wmo.Rotation.x),
        DegToRad(wmo.Rotation.z));

    if (isGlobalWmo)
        wmoPosition += Vec3D(533.33333f * 32, 533.33333f * 32, 0.0f);
    ...
    auto extractSingleSet = [&](WMO::MODS const& doodadSetData)
    {
        for (uint16 doodadIndex : doodadData.References)
        {
            ...
            std::snprintf(ModelInstName, sizeof(ModelInstName), "%s",
                          GetPlainName(&doodadData.Paths[doodad.NameIndex]));
            uint32 nlen = strlen(ModelInstName);
            ...
            Vec3D localPos{doodad.Position.x, doodad.Position.y, doodad.Position.z};
            Vec3D position = wmoPosition + (wmoRotation * localPos);

            Mat3 doodadRot = Mat3FromQuaternion(
                doodad.Rotation.X, doodad.Rotation.Y, doodad.Rotation.Z, doodad.Rotation.W);
            Mat3 combined = doodadRot * wmoRotation;

            Vec3D rotation;
            Mat3ToEulerAnglesXYZ_G3D(combined, rotation.z, rotation.x, rotation.y);

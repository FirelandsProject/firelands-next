...
/// Cataclysm 4.3.4 `SpellCastTargetFlags` (subset; see TCPP SharedDefines.h).
...
/// Client `SendCastFlags` on CMSG_CAST_SPELL / spell cast request (TCPP SpellPacketsCommon).
enum ClientSendCastFlags : uint8 {
  CLIENT_CAST_FLAG_HAS_TRAJECTORY = 0x02,
  CLIENT_CAST_FLAG_HAS_WEIGHT = 0x08,
};

/// Server `CastFlags` on SMSG_SPELL_* (TCPP Spell.h).
...
/// Parses CMSG_CAST_SPELL payload after opcode (TCPP `operator>> SpellCastRequest` + target).
...
/// Builds SMSG_SPELL_START (no HitInfo; matches TCPP `Spell::SendSpellStart` for common case).
